...
const allowlistedOrphans = new Set([
    /* Console-loadable compatibility module; active movement uses utility.Travel.Creep. */
    'Traveler.js'
]);
...
function findCycles(file, stack) {
    if (visiting.has(file)) {
        cycles.push(stack.slice(stack.indexOf(file)).concat(file).join(' -> '));
        return;
    }
...
console.log(`PASS: ${files.length} modules resolve; ${reachable.size} reachable; ${orphans.length} allowlisted orphan; 0 cycles`);
if (orphans.length) console.log(`Allowlisted orphan modules: ${orphans.join(', ')}`);
