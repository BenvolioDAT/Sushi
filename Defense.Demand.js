...
function getDemand(room) {
    ...
    const primaryRecord = threat && threat.hostiles.filter(record => record.autoEngage)
    ...
    const requiredDamage = threat && threat.actionableHostileCount > 0 ?
        Math.max(0, threat.actionableHealing + threat.actionableEffectiveHits / 10 - towerDamage) : 0;
    const requiredHealing = threat ? Math.max(0, (threat.actionableMelee + threat.actionableRanged) * 0.6) : 0;
    ...
    if (threat && threat.actionableHostileCount > 0 && requiredDamage > 0) {
        desiredRanged = Math.min(3, Math.max(1, Math.ceil(requiredDamage / rangedBodyPower)));
        if (threat.actionableDismantle > 0 && threat.actionableMelee > threat.actionableRanged) desiredMelee = 1;
    ...
    const demand = {
        tick: Game.time,
        roomName: room.name,
        operationId: threat && threat.actionableHostileCount > 0 ? `defend:${room.name}` : null,
        harmfulHostileCount: threat ? threat.actionableHostileCount : 0,
